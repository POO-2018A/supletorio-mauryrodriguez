...
public class InicioVentas extends javax.swing.JFrame {
    ...
    public InicioVentas() {
        initComponents();
        this.setTitle("Artículos de Computadora >> MAURY COMPANY <<");
    ...
    private void btn_removeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_removeActionPerformed
        limpiarHistorial();
        int indice[] = lst_productoCompra.getSelectedIndices();
        for (int i = indice.length-1; i >= 0; i--) {
            Cls_Producto pro = (Cls_Producto) listaCompra2.get(indice[i]);
            total -= pro.getPrecio();
            txt_total.setText(String.valueOf(total));
            pedidoProducto.remove(pro);
            listaCompra2.remove(indice[i]);
            if (total == 0) {
        ...
        if (cmb_clienteVenta.getSelectedIndex() != -1) {
            ...
            for (int i = 0; i < listaRegistroVenta.size(); i++) {
                ...
                if (clienteCombo.getNombre().equals(clienteArray.getNombre())) {
                    for (int j = 0; j < clienteArray.getListaCompras().size(); j++) {
